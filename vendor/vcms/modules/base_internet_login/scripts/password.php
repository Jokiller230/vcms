...
if(isset($_POST['email']) && $_POST['email'] != '' &&
	...
	if($libForm->wasCaptchaSolved($_POST['h-captcha-response'])){
		if(!$libString->isValidEmail($_POST['email'])){
			...
			echo '<h1>Fehler</h1>';
			$libGlobal->errorTexts[] = '<p class="mb-4">Die Nachricht konnte nicht verschickt werden. Bitte stellen Sie sicher, dass sie ein Mensch sind.</p>';
	}
}
...
echo '<form id="h-captcha-protected-form" action="index.php?pid=password" method="post" class="form-horizontal">';
echo '<fieldset>';
